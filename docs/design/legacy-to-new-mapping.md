...
| `auth.AuthenticationService`, `AuthenticationController`, registration, login, verification, and password requests | Authentication and account-recovery use cases | Identity — registration, authentication, and recovery | `RETAIN CONCEPT`, `REDESIGN` | Rebuild with normalized identity, non-enumerating responses, hashed single-use tokens, refresh rotation, revocation, throttling, and stable errors in FH-011 through FH-013 |
| `user.Token` and `TokenRepository` | Expiring verification or recovery token behavior | Identity — purpose-specific security tokens and sessions | `SPLIT`, `REDESIGN` | Verification, recovery, and refresh tokens have different invariants. Store only protected token material and model each lifecycle explicitly in FH-011 through FH-013 |
| `security.JwtService`, `JwtFilter`, `SecurityConfig`, and `UserDetailsServiceImpl` | Existing authentication flow and protected-route scenarios | Identity/security adapters | `REFERENCE ONLY` | Do not copy the custom JWT implementation. Rebuild using the approved Spring Security and Nimbus conventions, short-lived access tokens, refresh rotation, and server-side Provider permission resolution in FH-012 and FH-014 |
| `role.Role`, `RoleRepository`, `User.roles`, and `UserType` | Distinction between customer, administrator, owner, and staff behavior | Identity — Global Role; Provider — Membership and Provider Role Grant | `SPLIT`, `REDESIGN` | Only platform-wide roles remain global. `OWNER`, `BRANCH_MANAGER`, `RECEPTIONIST`, `TECHNICIAN`, and `ACCOUNTANT` are Provider participation roles evaluated through Membership in FH-014 and FH-020 |
...
| `email.EmailService` and `EmailTemplateName` | Email delivery scenarios | Identity use cases through a delivery port; Communication delivery records where applicable | `REDESIGN` | Preserve the need for verification and recovery email, but separate business commands, templates, outbox reliability, and provider adapters. Begin in FH-011 |
