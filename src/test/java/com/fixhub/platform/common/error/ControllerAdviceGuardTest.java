...
class ControllerAdviceGuardTest {
    ...
    void exactlyOneOwnControllerAdviceExists() {
        Map<String, Object> advices =
                applicationContext.getBeansWithAnnotation(ControllerAdvice.class);
                        ...
                        .filter(
                                entry ->
                                        entry.getValue()
                                                .getClass()
                                                .getName()
                                                .startsWith(OWN_BASE_PACKAGE))
                        .map(Map.Entry::getKey)
                        .collect(Collectors.toSet());

        assertThat(ownAdvices)
                .as(
                        "Expected exactly one @ControllerAdvice of ours (GlobalExceptionHandler) but found "
                                + "%s. A second advice makes @ExceptionHandler resolution between advices "
                                + "ambiguous and unpredictable across Spring versions.",
                        ownAdvices)
                .hasSize(1);
    }
}
